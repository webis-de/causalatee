...
Meant to run right after a ``conversion_script.py`` (or the shared :class:`~causalatee.data.conversion.FormatConverter`
...
classes turn up; nothing about :func:`verify_dataset`'s signature needs to
change for that.
"""
...
def _check_entity_markup(text_col: Sequence[str], relations_col: Sequence[Sequence[Mapping]]) -> list[str]:
    ...
    Corrupted or truncated markup can name an entity id in ``relations`` that :func:`~.markers.parse_entity_markers`
    never recovers a span for -- this would otherwise KeyError deep in a downstream consumer's span lookup instead
    of failing here with a clear cause.
    """
    errors: list[str] = []
...
def verify_dataset(batch: Mapping[str, Sequence], task: Task) -> list[str]:
    ...
    ``batch`` is column-oriented (``batch["text"]``, ``batch["relations"]``, ...) -- same convention as
    :mod:`.batch`'s ``Dataset.map``-ready functions, so a ``pandas.DataFrame`` works directly by column name right
